...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const title = post?.title || 'Technical Article';
  const description = post?.excerpt || "Read this article on Anuj Yadav's developer portfolio.";
  const category = post?.category?.name || 'ESSAY';
  const readTime = post?.readingTimeMinutes ? `${post.readingTimeMinutes} min read` : '';
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <span style={{ fontSize: '16px', fontWeight: 700, color: '#ffffff' }}>
              {post?.author?.displayName || 'Anuj Yadav'}
            </span>
