...
export default async function Image({
  ...
  const description =
    project?.shortDescription || 'Explore this architecture case study and codebase.';
  const category = project?.category?.name || 'CASE STUDY';
  const techStack = project?.technologies?.slice(0, 4).join(' • ') || '';
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <span style={{ fontSize: '13px', color: '#71717a' }}>
                Full-Stack Case Study &amp; Architecture
              </span>
            </div>
          </div>
